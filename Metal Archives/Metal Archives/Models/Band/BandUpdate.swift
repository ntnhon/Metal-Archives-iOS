...
final class BandUpdate: BandAdditionOrUpdate, Pagable {
    ...
    static func parseListFrom(data: Data) -> (objects: [BandUpdate]?, totalRecords: Int?)? {
        ...
        listBandLatestAdditions.forEach { (bandDetails) in
            ...
            if let bandAddition = BandUpdate(updatedDateString: updatedDateString, name: name, urlString: urlString, countryURLString: countryURLString, genre: genre, updatedDateAndTimeString: updatedDateAndTimeString) {
