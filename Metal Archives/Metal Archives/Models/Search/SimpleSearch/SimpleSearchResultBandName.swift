...
final class SimpleSearchResultBandName: SimpleSearchResultBandNameOrMusicGenre, Pagable {
    ...
    static func parseListFrom(data: Data) -> (objects: [SimpleSearchResultBandName]?, totalRecords: Int?)? {
        var list: [SimpleSearchResultBandName] = []
        ...
        listSimpleSearchResultBandName.forEach { (eachDetail) in
            ...
            if let `band` = band, let `genre` = genre, let `country` = country {
                if let result = SimpleSearchResultBandName(band: band, aka: aka, genre: genre, country: country) {
                    list.append(result)
                }
            }
        }
